...
router.post('/addUser', (req, res) => {
    var sql = $sql.student_user.add;
    var params = req.body;
    console.log(params);
    // console.log(params.birth);
    conn.query(sql, [params.studentName, params.studentID, params.passWord, params.checkPassWord], function(err, result) {
        if (err) {
            console.log(err);
        }
        if (result) {
...
router.post('/login', (req, res) => {
    var sql_name = $sql.student_user.select_name;
    // var sql_password = $sql.user.select_password;
    var params = req.body;
    var keywords = JSON.parse(Object.keys(params)[0]);
    if (keywords.studentID) {
        sql_name += " where studentID ='"+ keywords.studentID +"'";
        console.log(sql_name);
    }
    conn.query(sql_name, keywords.studentID, function(err, result) {
        if (err) {
            console.log(err);
        }
        // console.log(result);
...
router.get('/getUser', (req, res) => {
    ...
    console.log(params);
    if (params.studentID) {
        sql_name += "where studentID ='"+ params.studentID+"'";
    }
    conn.query(sql_name, params.studentID, function(err, result) {
        if (err) {
            console.log(err);
        }
        // console.log(result);
...
router.post('/updateUser', (req, res) => {
    var sql_update = $sql.student_user.update_user;
    var params = req.body;
    console.log(params);
    if (params.id) {
